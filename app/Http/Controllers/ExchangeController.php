...
public function index(){
 ...
 $checkExistsopenshift['treasuries_balance_now'] = Treasuries_transactions::where(['com_code'=>$com_code,'shift_code'=>$checkExistsopenshifts['shift_code']])->sum('money');
    ...
    return view('admin.exchange_transactions.index',['data'=>$data ,'checkExistsopenshifts'=>$checkExistsopenshifts,'accounts'=>$accounts,'mov_type'=>$mov_type , 'checkExistsopenshift'=>$checkExistsopenshift]);
}
...
public function store(exchange_transactionRequest $request){
    try{
        ...
        $trasury_data = Treasuries::select('last_isal_exchange')->where(['com_code'=>$com_code,'id'=>$request->treasuries_id])->first();
        ...
        $data_Insert['isal_number']= $trasury_data['last_isal_exchange'] + 1 ;
        ...
        if($flag){
            ...
            $dataUpdateTresuries['last_isal_exchange'] = $data_Insert['isal_number'];
             Treasuries::where(['com_code'=>$com_code,'id'=>$request->treasuries_id])->update($dataUpdateTresuries);
        return redirect()->route('admin.exchange_transaction.index')->with(['success'=>'تم اضافة البيانات بنجاح']);
        } else{
