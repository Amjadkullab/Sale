...
try{
    ...
    $data = Treasuries::select()->find($id);
    if(empty($data)){
        return redirect()->route('admin.treasuries.index')->with(['error' => 'عفوا غير قادر على الوصول الى البياات المطلوبة !!']);
}
...
public function details($id){
  ...
  if ($data['updated_by'] > 0 and $data['updated_by']!= null) {
    ...
    $treasuries_delivery = TreasuriesDelivery::select()->where(['treasuries_id'=>$id])->Orderby('id','DESC')->get(); // treasuries_id هي الخزنة الاب
...
public function store_treasuries_delivery($id,Addtreasuries_deliveryRequest $request ){
    ...
    try{
        ...
        $data = Treasuries::select('id','name')->find($id);
        if(empty($data)){
            return redirect()->route('admin.treasuries.index')->with(['error' => 'عفوا غير قادر على الوصول الى البيانات المطلوبة !!']);
    }
