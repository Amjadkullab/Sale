...
@section('title', 'تعديل بيانات فئة الموردين ')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title card_title_center">تعديل فئة المورد </h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                    @if (@isset($data) && !@empty($data))
                     <form action="{{ route('admin.supplier_categories.update', $data['id']) }}" method="post" ">
                      @csrf
                      <div class="form-group">
                        <label for="name">اسم فئة المورد</label>
                        <input  name="name" id="name" class="form-control" value="{{ old('name',$data['name']) }}" placeholder="أدخل اسم المورد" oninvalid="setCustomValidity('من فضلك أدخل هذا الحقل')" onchange="try{setCustomValidity('')}catch(e){}">
                         @error('name')
